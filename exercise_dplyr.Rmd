...
#note that "year" is not capitalized here because the previous change was not assigned to tan object
...
# remember that == compares by element 
# "c" creates a vector; here, it creates a vector within "country" for just China and Brazil and then looks at the population

gapminder |>
  filter(country %in% c("Brazil", "China") & pop > 100000000)

# another method!
allowed_countries <- c("Brazil", "China")
...
gapminder |>
  mutate(
    lifeExp = if_else(country == "Canada" & year == 1952, true = 70, false = lifeExp)
  )
```
...
#can also create new varible within group_by
...
# Advanced dplyr functions 2/8/2022 
...
#glimpse function is cute! puts data on side

glimpse(dat_bfi)
    
#recoding gender; old values = new values because mutate function is backwordss
...
dat_bfi$education |> unique()

#more tedious way to do this without using across function
...
#making own function; shortcut is backslash \

add1 <- function(x) x + 1
add1(3)

#writing function in across
...
#can do multiple functions in across; use list to create a pile of functions
...
#recoding with across; reverse coding scale items
...
#look at dictonary to see which items are reverse-scored
...
#putting everything together !; remember, the first argument inside across is columns; adding .names made new columns with new names instead of overriding previous data; {} calls on current column names and then adds r to indicate reverse score value
...
#use "?starts_with" to see the different selections available for the across function; such as selecting all the columns that contain "aggression" in their name

#using rowwise

#for one variable; use c_across because it allows you to average per row across multiple columns; with just c, you would have to add the A columns individually

#always want to ungroup before running onto another chunk of code when working with rowwise or group_by

dat_bfi |>
  rowwise() |> 
  mutate(
    .id = .id,
    A_total = mean(c_across(A1:A5), na.rm = TRUE), .before = "A1") |> 
  ungroup()
